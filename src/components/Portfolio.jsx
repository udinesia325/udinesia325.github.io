...
function Portfolio() {
  return (
    <div id='portfolio' className="pt-16 pb-10 portfolio-gradient">
      <h1 className="text-3xl font-semibold text-center lg:text-4xl" data-aos='fade-up'>Portfolio</h1>
      <p className="text-xs font-light text-gray-500 mt-4 text-center px-2 sm:px-0 lg:text-lg" data-aos='fade-up' data-aos-delay='200'>
      ...
      <div className="w-[90%] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3  gap-5 mx-auto mt-10">
        {portfolio.map((port, index) => (
          <div className="border rounded-md overflow-hidden bg-white flex flex-col items-start justify-start" key={index} data-aos='zoom-in' data-aos-delay={200 * index} data-aos-anchor-placement="bottom-bottom">
            <img
              src={port.img}
            ...
            <div className='p-3 lg:pb-5 flex-1 flex flex-col items-start justify-start'>
              <span className="text-xs text-gray-500 font-light lg:text-sm">
                {port.category}
              </span>
              <h1 className="font-semibold mt-[2px] text-lg">{port.title}</h1>
              <p className="text-sm text-gray-500 font-light mt-` mb-1 lg:mt-2 lg:mb-4">
                {port.description}
              </p>
              {port.isPrivate ? (
                <button className="text-primary-400 border bg-primary-100/50 border-primary-200 text-sm py-1 px-4 font-semibold min-w-[80px] transition-all flex items-center gap-2 cursor-not-allowed mt-auto">
                  Private WEB
