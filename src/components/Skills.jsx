...
function Skills() {
  return (
    <div className="w-full py-10 skill-gradient">
      <h1 className="text-center text-3xl font-semibold text-gray-900 lg:text-4xl" data-aos='fade-in'>
        Skills
      </h1>
      <p className="text-sm text-center mt-3 text-gray-500 lg:text-lg" data-aos='fade-in' data-aos-delay='100'>
        ...
        {skills.map((skill, index) => (
          <div className='basis-1/3 sm:basis-1/6 flex justify-center items-center'>
            <div className='group transition-all text-gray-500 hover:text-black relative text-4xl w-fit'>
              <p className='hidden md:hidden md:group-hover:block absolute -top-5 right-0 text-left bg-gray-500 text-white px-2 rounded-sm text-xs'>{skill.name}</p>
              <Icon
                icon={skill.icon}
                key={index}
